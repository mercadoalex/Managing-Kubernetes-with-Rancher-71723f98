...
Run these commands on the :tab{text='dev-machine' machine='dev-machine'} terminal, the same workstation you have been using. It has `git` and can reach Gitea over the network, so you work exactly as a developer would - from your own machine. There is no need to log in to the Gitea machine or the cluster.
