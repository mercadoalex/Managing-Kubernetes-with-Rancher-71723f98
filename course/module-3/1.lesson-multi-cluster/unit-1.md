...
You drive everything from the :tab[**dev-machine**]{name="dev-machine"} workstation, open the dashboard with the :tab[**Rancher**]{name="Rancher"} tab, and run the registration command on the downstream cluster through the :tab[**downstream-01**]{name="downstream-01"} terminal.
...
In the :tab[**dev-machine**]{name="dev-machine"} terminal, list the clusters Rancher knows about:
...
Copy the registration command Rancher displayed. Then switch to the :tab[**downstream-01**]{name="downstream-01"} terminal - this is the shell *on the downstream cluster itself* - and run it there.
...
This is the one place in the course where you deliberately work *on* a cluster node rather than from the workstation. The :tab[**downstream-01**]{name="downstream-01"} terminal is a shell on the downstream cluster's node, and its `kubectl` targets the downstream cluster. The :tab[**dev-machine**]{name="dev-machine"} terminal, by contrast, targets the upstream Rancher cluster. Keep the two straight: you apply the agent manifest on **downstream-01**, and you verify the result from **dev-machine** (or in the Rancher UI).
...
Confirm it from the :tab[**dev-machine**]{name="dev-machine"} terminal as well - Rancher created a management object for the imported cluster on the upstream:
